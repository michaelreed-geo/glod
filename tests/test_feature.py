...
from glod.geometry import LineString, Point, Polygon
...
def point_feature():
    return Feature(
        geometry=Point.from_geojson({"type": "Point", "coordinates": [1, 2]}, crs="epsg:27700"),
...
def line_feature():
    return Feature(
        geometry=LineString.from_geojson({"type": "LineString", "coordinates": [[0, 0], [1, 1]]}, crs="epsg:27700"),
        attributes={"id": 1},
    )
...
class TestFeatureInit:
    ...
    def test_attributes_default_to_empty_dict(self):
        f = Feature(geometry=Point.from_geojson({"type": "Point", "coordinates": [1, 2]}))
        assert f.attributes == {}

    def test_attributes_are_copied(self):
        original = {"x": 1}
        f = Feature(geometry=Point.from_geojson({"type": "Point", "coordinates": [1, 2]}), attributes=original)
        f.attributes["x"] = 99
...
class TestFeatureSetters:
    def test_geometry_setter_accepts_geometry(self, point_feature):
        new_geom = Point.from_geojson({"type": "Point", "coordinates": [9, 9]}, crs="epsg:27700")
...
class TestFeatureEquality:
    def test_equal_features(self):
        f1 = Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}), {"a": 1})
        f2 = Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}), {"a": 1})
        assert f1 == f2

    def test_different_geometry(self):
        f1 = Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}), {"a": 1})
        f2 = Feature(Point.from_geojson({"type": "Point", "coordinates": [3, 4]}), {"a": 1})
        assert f1 != f2

    def test_different_attributes(self):
        f1 = Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}), {"a": 1})
        f2 = Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}), {"a": 2})
        assert f1 != f2
...
class TestFeatureFromGeoJSON:
    ...
    def test_null_geometry_raises(self):
        with pytest.raises(ValueError, match="geometry"):
            Feature.from_geojson({"type": "Feature", "geometry": None, "properties": {}})
...
class TestFeatureCollectionSetters:
    def test_features_setter_accepts_list_of_features(self, point_feature, line_feature):
...
class TestFeatureCollectionAppendRemove:
    def test_append_feature(self, simple_fc, point_feature):
        ...
        new_f = Feature(Point.from_geojson({"type": "Point", "coordinates": [5, 6]}, crs="epsg:27700"), {})
    ...
    def test_remove_absent_feature_raises(self, simple_fc):
        absent = Feature(Point.from_geojson({"type": "Point", "coordinates": [99, 99]}), {})
...
class TestFeatureCollectionCRS:
    ...
    def test_mixed_crs_returns_none(self):
        fc = FeatureCollection([
            Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}, crs="epsg:27700"), {}),
            Feature(Point.from_geojson({"type": "Point", "coordinates": [0, 51]}, crs="epsg:4326"), {}),
        ])
    ...
    def test_all_none_crs_returns_none(self):
        fc = FeatureCollection([
            ...
            Feature(Point.from_geojson({"type": "Point", "coordinates": [3, 4]}), {}),
        ])
        assert fc.crs is None
    ...
    def test_crs_updates_when_feature_geometry_changes(self):
        ...
        fc[0].geometry = Point.from_geojson({"type": "Point", "coordinates": [1, 2]}, crs="epsg:4326")
...
class TestFeatureCollectionFromGeoJSON:
    ...
    def test_wrong_type_raises(self):
        with pytest.raises(ValueError, match="FeatureCollection"):
            FeatureCollection.from_geojson({"type": "Feature", "geometry": None, "properties": {}})
...
class TestFeatureCollectionFromCSV:
    ...
    def test_basic(self):
        path = self._write_csv(
            [{"geometry": "POINT (1 2)", "name": "A"}, {"geometry": "POINT (3 4)", "name": "B"}],
...
class TestFeatureCollectionToCSV:
    ...
    def test_sparse_attributes_written(self):
        fc = FeatureCollection([
            Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}), {"a": 1}),
            Feature(Point.from_geojson({"type": "Point", "coordinates": [3, 4]}), {"b": 2}),
        ])
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as tmp:
            path = tmp.name
        try:
...
class TestNormaliseCRS:
    ...
    def test_uniform_crs_returns_original_list(self):
        features = [
            Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}, crs="epsg:27700"), {}),
            Feature(Point.from_geojson({"type": "Point", "coordinates": [3, 4]}, crs="epsg:27700"), {}),
        ]
    ...
    def test_mixed_crs_pyproj_enabled_transforms(self):
        ...
        features = [
            Feature(Point.from_geojson({"type": "Point", "coordinates": [1, 2]}, crs="epsg:27700"), {"id": 1}),
            Feature(Point.from_geojson({"type": "Point", "coordinates": [3, 4]}, crs="epsg:4326"), {"id": 2}),
        ]
